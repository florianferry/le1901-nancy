...
if input_tokens > 0 or cache_create > 0 or cache_read > 0 or output_tokens > 0:
    print(json.dumps({
        ...
        "description": "auto",
        "project": "/Users/florianferry/Développement/le1901-nancy-main",
    }))
PYEOF
)
  if [[ -n "$USAGE" ]]; then
    # POST to MCP graph server (always running, reliable)
    MCP_PORT=$(cat "/Users/florianferry/Développement/le1901-nancy-main/.dual-graph/mcp_port" 2>/dev/null || echo "8081")
